...
double yParameterSensor(double strip, int sensor)
{
	if (sensor == 0 || sensor == 5 || sensor == 4) // kpix side showing towards beam movement beam side  KPIX >  < Beam
	{
		...
		else return 50*(-strip+1840); // sensor 0,1, 3 and 4 have a 2 degree stereo angle means the y movement is slightly lower (50*cos(+-2/57.295))
	...
	else  // kpix side in direction of beam movement KPIX < < BEAM
	{
		...
		else return 50*strip; // sensor 0,1, 3 and 4 have a 2 degree stereo angle means the y movement is slightly lower (50*cos(+-2/57.295))
	}
}
